mod highwayhash;
pub use self::highwayhash::{highway_hash128, highway_hash256, highway_hash64, HighwayHashState};

#[cfg(test)]
mod tests {
    use super::*;

    const K_MAX_SIZE: usize = 64;

    fn test_hash64(expected: u64, data: &Vec<u8>, key: [u64; 4]) {
        let hash = highway_hash64(data, key);
        println!("hash require: {}, value : {}",  expected, hash);
    ...
    fn test() {
        ...
        let test_key2: [u64; 4] = [1u64, 2u64, 3u64, 4u64];

        let expected64: [u64; K_MAX_SIZE + 1] = [
        ...
        let mut data: [u8; K_MAX_SIZE + 1] = [0; K_MAX_SIZE + 1];

        for i in 0..K_MAX_SIZE {
            data[i] = i as u8;
        ...
        test_hash64(0x53c516cce478cad7u64, &v, test_key2);
    }
}
